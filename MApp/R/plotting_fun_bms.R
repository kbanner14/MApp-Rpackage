...
#' @title Model Averaged Posteriors Plot (MAP plot) - dark theme 
#' @description same as \code{MApp_bms} but with a dark background. 
#'   For use with \code{bma} objects obtained using the \code{bms} function in
#'   the \code{BMS} package. Creates the model averaged posteriors plot, displaying 
#'   posteriors of regression coefficients from individual models along with the 
#'   corresponding model averaged posteriors. Used to visually display the components 
#'   of model averaged partial regression coefficients and assess appropriateness of 
...
#'   for the top \code{max_display} individual models specified, along with the
#'   posterior standard deviation of the model averaged parameter.
MApp_bms_dark <- function(x, plot_wind, num_sims = 1000,
                     max_display = NULL, mod_names = NULL,
                     include_coef = NULL, ...) {
  # define pipe 
  `%>%` <- magrittr::`%>%`
  ...
  # make the beanplot
  par(mfrow = c(plot_wind), las = 1, mar = c(2.1,3.1,3.1,.5), fg = "#f0f0f0", 
      col.axis = "#f0f0f0", col.lab = "#f0f0f0")
  for (i in include_coef) {
    ...
    ma.mean[is.na(ma.mean)] <- 0
    MA.mean <- mean(ma.mean)
    segments(MA.mean, 1, MA.mean, (max_display + 1.35), col = "#f0f0f0", lty = 4)
